...
\textbf{Definition 1}: We will denote an $2$ player game $$\Gamma_2 = (S, \boldsymbol{u}, \mathcal{R}, \mathcal{N}, \tau)$$
\begin{enumerate}
\item The strategy profile of the game $$S = \displaystyle \prod_{i = 1}^2 S_i = S_1 \times S_2$$ where $S_i$ is the strategy space of player $i$.
\item $u_i$ is the utility function for each player $i$, $$u_i \mathpunct{:} S \to \mathbb{R} \implies \boldsymbol{u} = \langle u_1, u_2 \rangle$$
Our game is a zero sum game where $u_2 = -u_1$. 
\item $\mathcal{N}$ is the network
\item $\mathcal{R}$ is the resource capacity
\item $\tau$ is the network topology
\end{enumerate}
...
\textbf{Definition 2}: Player strategies \\
Let $|\mathcal{V}|$ denote the order of $\mathcal{N}$. \\
Each player strategy is a vector $\boldsymbol{s_i} \in S_i$ is a vector in $\mathbb{Z}^{|\mathcal{V}| + 1}$
\begin{enumerate}
\item $\boldsymbol{s_i} = \langle s_i^1, s_i^2, \ldots, s_i^{|\mathcal{V}|} \rangle$
\item $\displaystyle \sum_{k = 1}^{|\mathcal{V}|} s_i^k = \mathcal{R}$
\end{enumerate}
\end{enumerate}

\textbf{Definition 3}: Pure strategy profiles \\
A strategy profile $\boldsymbol{s} \in S$ is a vector $$\boldsymbol{s} = \langle\boldsymbol{s_1}, \boldsymbol{s_2}\rangle$$ where $\boldsymbol{s_i}$ is the strategy of player $i$.

\par A mixed strategy for a player is an act of selecting a strategy according to a probability distribution. Let $\Delta S_i$ denote the set of all probability distributions over the strategies of player $i$.

\textbf{Definition 4}: A mixed strategy $\boldsymbol{\sigma_i} \in \Delta S_i$ is a vector $$\boldsymbol{\sigma_i} = \langle\sigma_i^1, \sigma_i^2, \ldots, \sigma_i^{|S_i|}\rangle$$ where $\sigma_i^j = \boldsymbol{\sigma_i}(\boldsymbol{s_j})$ is the probability that player $i$ will play strategy $\boldsymbol{s_j}$.
