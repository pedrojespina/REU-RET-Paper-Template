...
In this section we define a general game model to serve as the bais for the rest of the games.
...
\item A pure strategy profile $\vec{s} \in S$ is a \textbf{Nash equilibrium} if and only if $$u_i(\vec{s}_i, \vec{s}_{-i}) \geq u_i(\vec{s'}_i, \vec{s}_{-i}) \forall{\vec{s'}_i \in S_i}$$
\item A mixed strategy profile $\vec{\sigma_i} \in \Delta S_i$ is a \textbf{mixed Nash equilibrium} if and only if $$u_i(\vec{\sigma}_i, \vec{\sigma}_{-i}) \geq u_i(\vec{\sigma'}_i, \vec{\sigma}_{-i}) \forall{\vec{\sigma'}_i \in \Delta S_i}$$

\end{enumerate}

\section{Static Colonel Blotto Game Model}

In this section we write the model for a game where an attacker and defender compete for control of nodes in a network. 

\textbf{Definition 9}: We will denote an $2$ player game $$\Gamma_2 = (\mathcal{P}, S, u, \mathcal{N}, \tau)$$
\begin{enumerate}
